...
from .dbutil import LockedConnection
...
class MemoryStore:
    ...
    def __init__(self, path: str = ":memory:", max_active_rules: int | None = None):
        # The application server answers requests on a threadpool, so one
        # store is created on the startup thread and used from many worker
        # threads. Two things are needed for that and only the first is
        # obvious.
        #
        # check_same_thread=False lifts sqlite3's own thread check.
        # MEASURED: that alone is NOT enough. With the check lifted and
        # eight threads writing, sqlite3 raised
        # OperationalError('not an error') -- the classic symptom of two
        # threads driving one connection at once, and a misleading message
        # for a real race. sqlite3.threadsafety reports 3 (SERIALIZED) on
        # this build and it still happened, because the race is in the
        # Python layer rather than in SQLite.
        #
        # So every statement goes through _LockedConnection, which holds a
        # lock across execute AND the fetch, and hands back rows that are
        # already materialised. That covers callers outside this class too
        # -- the server reads self.db directly.
        self.db = LockedConnection(sqlite3.connect(path, check_same_thread=False))
